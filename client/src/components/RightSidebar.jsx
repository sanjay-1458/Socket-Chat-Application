...
function RightSidebar({selectedUser}) {
  return selectedUser && (
    <div className={`bg-[#818582]/10 text-white relative overflow-y-scroll ${selectedUser?
      "max-md:hidden":""
    }`}>
      ...
      <div className='px-5 text-xs'>
        ...
        <div className='mt-2 max-h-[70vh] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {imagesDummyData.map((url,index)=>{
            return <div key={index} onClick={()=>window.open(url)} className='cursor-pointer rounded'>
              <img src={url} className='h-full rounded-lg'/>
            </div>
          })}
        </div>
      </div>
